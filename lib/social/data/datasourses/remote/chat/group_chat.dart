...
class FireStoreGroupChat {
  ...
  static Future<void> deleteChat(
      {required String chatOfGroupUid, required Message messageInfo}) async {
    final fireStoreUserCollection = FirebaseFirestore.instance.collection('users');
    //await _fireStoreChatCollection.doc(chatOfGroupUid).delete();
    DocumentSnapshot<Map<String, dynamic>> fireChatsCollection = await fireStoreUserCollection.doc(myPersonalId).get();
    Map<String, dynamic>? data = fireChatsCollection.data();
    List<dynamic> myChatOfGroup = await fireChatsCollection.get("chatsOfGroups");
    myChatOfGroup.remove(chatOfGroupUid);
    data!["chatsOfGroups"] = myChatOfGroup;
    await fireStoreUserCollection.doc(myPersonalId).set(data);
  }
}
